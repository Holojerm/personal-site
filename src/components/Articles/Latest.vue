...
<template>
  <router-link :to="to" class="h-full inline-grid gap-0 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2">
    ...
    <div class="mt-5 flex flex-col lg:px-5 lg:pt-5 lg:mt-0">
      <span
        class="mb-2 sm:mt-5 lg:mt-0 text-bittersweet-700 dark:text-bittersweet-50"
      >Posted at {{ $props.date }}</span>
      <router-link
        :to="to"
        class="text-3xl text-bittersweet-800 dark:text-orient-400 font-bold mb-2"
      >
        {{
          $props.title
        }}
      ...
      <p
        class="text-sm font-light mb-5 text-bittersweet-700 dark:text-bittersweet-50"
      >{{ $props.description }}</p>
